...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            h, cache_lstm = lstm_forward(word_vector, h0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        prev_word_vector = self._start * np.ones((N,), dtype=np.int32)
        for i in range(max_length):
            prev_words = W_embed[prev_word_vector]
            if self.cell_type == 'rnn':
                h, _ = rnn_step_forward(prev_words, h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                h, cell, _ = lstm_step_forward(prev_words, h, cell, Wx, Wh, b)
            scores, _ = affine_forward(h, W_vocab, b_vocab)
            prev_word_vector = np.argmax(scores, axis=1)
            captions[:,i] = prev_word_vector
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
